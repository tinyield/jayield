 ...
 */
public class TraversableFilter<T> implements Traversable<T> {

    private final Traversable<T> src    ;
    private final Predicate<T> p;

    public TraversableFilter(Traversable<T> src, Predicate<T> p) {
        this.src = src;
        this.p = p;
    }
    ...
    public Advancer<T> advancer() {
        return AdvancerExtensions.filter(src.advancer(), p);
    }
}
