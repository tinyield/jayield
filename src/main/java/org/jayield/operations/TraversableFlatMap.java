 ...
 */
public class TraversableFlatMap<T, R> implements Traversable<R>{
    private final Traversable<T> src;
    private final Function<T, Traversable<R>> mapper;

    public TraversableFlatMap(Traversable<T> src, Function<T, Traversable<R>> mapper) {
        this.src = src;
        this.mapper = mapper;
    }
    ...
    public Advancer<R> advancer() {
        return AdvancerExtensions.flatMap(src.advancer(), mapper);
    }
}
